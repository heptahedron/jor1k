export default class EventHandler {
  ...
  on(type, handler) {
    if (!this.eventTypeHandlers[type]) {
      this.eventTypeHandlers[type] = [];
    }
    this.eventTypeHandlers[type].push(handler);
  }
  ...
  emit(type, data) {
    ...
    this.eventTypeHandlers[type].forEach(handler => handler(data));
  }
}
